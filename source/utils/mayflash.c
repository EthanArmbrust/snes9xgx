...
void Mayflash_ScanPads()
{
	...
	// you have to make 2 calls to get the status, even if you are only interested in one port
	...
	// X=6,1F										fixed
	// Y=5,8F										good
	// Select=6,10 ; Hornet button label = "9"		good
	// Start=6,20  ; Hornet button label = "10"		good
	// Up=4,00										good
	// Right=3,FF									good
	// Left=3,00									good
	// Down=4,FF									good
	...
	jp |= ((buf[6] & 0x10) == 0x10) ? PAD_TRIGGER_Z : 0; // Mayflash select button maps to Z
